...
- Separator lines: three or more minuses on their own line (`---`, `----------`,
  also `***` / `___` and spaced `- - -`) now render as a horizontal rule, with
  the raw markers revealed again when the cursor is on the line. Near-misses
  stay plain text: two minuses, markers with trailing text, minuses mid-line,
  `---` under a paragraph (that is a setext heading), and anything inside a
  fenced code block.
